...
import '../styles/DrinkDescription.css'
...
class DrinkDescription extends Component {
  ...
  render() {
    return (
      <div>
        <Button style={{marginTop: 0}} className="desc-btn"  onClick={this.handleClick}><span style={{fontSize: '1.5rem'}}>{(this.state.button===true) ? '-' : '+'}</span><span> Description</span>
        </Button>
        {
          this.state.hideable &&
